# Angles++ : Walking model for correcting the bad frames - LCR-NET
# Usage: python3 correcte_lcr.py <nb_video> <direction(front/back)> <filter(gaussian/mean/median)> <filter kernel size> <showfigures> ex : 1 front gaussian 5 True')
...
if __name__=='__main__':
    ...
    if(len(sys.argv)!=6):
        print('usage: python3 correcte_lcr.py <nb_video> <direction(front/back)> <filter(gaussian/mean/median)> <filter kernel size> <showfigures> ex : 1 front gaussian 5 True')
    ...
    for i in range(len(k['frames'])):
        k['kangle_l'][i]=180-tools_lcr.angle((k['x_lak'][i],k['y_lak'][i],k['z_lak'][i]),(k['x_lkn'][i],k['y_lkn'][i],k['z_lkn'][i]),(k['x_las'][i],k['y_las'][i],k['z_las'][i]), False)
        k['kangle_r'][i]=180-tools_lcr.angle((k['x_rak'][i],k['y_rak'][i],k['z_rak'][i]),(k['x_rkn'][i],k['y_rkn'][i],k['z_rkn'][i]),(k['x_ras'][i],k['y_ras'][i],k['z_ras'][i]), False) 
    fig4=plt.figure()
    ...
    if direction=='back':
        k.to_csv('testVideos'+'/test'+nb_video+'/'+number+'_LCR-NET_joints_3DKinect_back3.csv',encoding='gbk')
    else:
        k.to_csv('testVideos'+'/test'+nb_video+'/'+number+'_LCR-NET_joints_3DKinect_front.csv',encoding='gbk')
