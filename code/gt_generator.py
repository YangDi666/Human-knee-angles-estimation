# GT files generator
# !You should run this script after space calibration 
# Usage : python3 gt_generator.py <nb_video> <direction(f/b)> <frame_begin> <frame_end> <algo> <showfigures> the begin and end should be always the same with "show_result.py", "time_recalage.py" and "space_recalage.py"
...
print("Usage : python3 gt_generator.py <nb_video> <direction(f/b)> <frame_begin> <frame_end> <algo> <showfigures>")
...
if direction=='f':
    dt=jsondataT['dt']
    with open('testVideos/test'+nb_video+'/space_recalage_front_'+algo+'.json') as json_data:
        jsondataS = json.load(json_data)
else:
    dt=jsondataT['dt']
    with open('testVideos/test'+nb_video+'/space_recalage_back_'+algo+'.json') as json_data:
        jsondataS = json.load(json_data)
print('dt:', dt)
field=data2['field']
...
for arts in articulations:
    ...
    gt_rz=data2['Z_'+arts]   
    t1=time[frames[0]:frames[1]]
    outlx=tools_lcr.compt(t1, t1, field, gt_lx, dt)
    outly=tools_lcr.compt(t1, t1, field, gt_ly, dt)
    outlz=tools_lcr.compt(t1, t1, field, gt_lz, dt)
    ...
    #print(outl)
    outrx=tools_lcr.compt(t1, t1, field, gt_rx, dt)
    outry=tools_lcr.compt(t1, t1, field, gt_ry, dt)
    outrz=tools_lcr.compt(t1, t1, field, gt_rz, dt)
    gtrx=outrx[2]
    gtry=outry[2]
    gtrz=outrz[2]
...
for joint in jsonfinal.keys():
    if joint!='frame' and joint.find('ag')==-1 and joint.find('dis')==-1 and joint.find('step')==-1:
        ...
        for jf in jsonfinal[joint]:
            jf=tools_lcr.VIto3D(jf, jsondataS, joint[0], joint[2:])
            new_pos.append(jf)
...
for i in range(len(tfl)):
    jsonfifinal['kangle_l'].append(180-tools_lcr.angle((jsonfifinal['x_las'][i],jsonfifinal['y_las'][i],jsonfifinal['z_las'][i]),(jsonfifinal['x_lkn'][i], jsonfifinal['y_lkn'][i],jsonfifinal['z_lkn'][i]),(jsonfifinal['x_lak'][i],jsonfifinal['y_lak'][i],jsonfifinal['z_lak'][i])))
    jsonfifinal['kangle_r'].append(180-tools_lcr.angle((jsonfifinal['x_ras'][i],jsonfifinal['y_ras'][i],jsonfifinal['z_ras'][i]),(jsonfifinal['x_rkn'][i], jsonfifinal['y_rkn'][i],jsonfifinal['z_rkn'][i]),(jsonfifinal['x_rak'][i],jsonfifinal['y_rak'][i],jsonfifinal['z_rak'][i])))

data=pd.DataFrame(jsonfifinal)
data.to_csv('testVideos'+'/test'+nb_video+'/'+number+'_GT-Vicon_'+algo+'_3DKinect_'+direction+'_respectiveTF.csv', encoding='gbk')
print('GT_Kinect file.csv', 'saved!')
...
for joint in jsonfinal.keys():
    if joint!='frame' and joint.find('ag')==-1 and joint.find('dis')==-1 and joint.find('step')==-1:
        new_pos_g=[]
        for jf in jsonfinal[joint]:
            jf=tools_lcr.VIto3D(jf, jsondataS, joint[0])
            new_pos_g.append(jf)
        jj=joint.replace('Y', 'z')
        jj=jj.replace('Z', 'y')
        jj=jj.replace('X', 'x')
...
for i in range(len(tfl)):
    jsonfififinal['kangle_l'].append(180-tools_lcr.angle((jsonfififinal['x_las'][i],jsonfififinal['y_las'][i],jsonfififinal['z_las'][i]),(jsonfififinal['x_lkn'][i], jsonfififinal['y_lkn'][i],jsonfififinal['z_lkn'][i]),(jsonfififinal['x_lak'][i],jsonfififinal['y_lak'][i],jsonfififinal['z_lak'][i])))
    jsonfififinal['kangle_r'].append(180-tools_lcr.angle((jsonfififinal['x_ras'][i],jsonfififinal['y_ras'][i],jsonfififinal['z_ras'][i]),(jsonfififinal['x_rkn'][i], jsonfififinal['y_rkn'][i],jsonfififinal['z_rkn'][i]),(jsonfififinal['x_rak'][i],jsonfififinal['y_rak'][i],jsonfififinal['z_rak'][i])))
    
data=pd.DataFrame(jsonfififinal)
data.to_csv('testVideos'+'/test'+nb_video+'/'+number+'_GT-Vicon_'+algo+'_3DKinect_'+direction+'_generalTF.csv', encoding='gbk')
print('GT_Kinect_general file.csv', 'saved!')
...
for joint in j:
    ...
    for i in range(len(tfl)):
        (x,y)=tools_lcr.K3DtoD((jsonfifinal['x_'+joint][i], jsonfifinal['y_'+joint][i], jsonfifinal['z_'+joint][i]))
        (x,y)=tools_lcr.DtoRGB((x,y))
        jsonfifififinal['x_'+joint].append(x)
        jsonfifififinal['y_'+joint].append(y)  
data=pd.DataFrame(jsonfifififinal)
data.to_csv('testVideos'+'/test'+nb_video+'/'+number+'_GT-Vicon_'+algo+'_2DColor_'+direction+'_respectiveTF.csv', encoding='gbk')
print('GT_Color_respective file.csv', 'saved!')
...
for joint in j:
    ...
    for i in range(len(tfl)):
        (x,y)=tools_lcr.K3DtoD((jsonfififinal['x_'+joint][i], jsonfififinal['y_'+joint][i], jsonfififinal['z_'+joint][i]))
        (x,y)=tools_lcr.DtoRGB((x,y))
        jsonfififififinal['x_'+joint].append(x)
        jsonfififififinal['y_'+joint].append(y)  
data=pd.DataFrame(jsonfififififinal)
data.to_csv('testVideos'+'/test'+nb_video+'/'+number+'_GT-Vicon_'+algo+'_2DColor_'+direction+'_generalTF.csv', encoding='gbk')
print('GT_Color_general file.csv', 'saved!')
...
ax1.set_title(algo+' - Left Ankle z (gt_r, Tfrespective_b, Tfgeneral_g)')
...
ax3.set_title(algo+' - Right Ankle z (gt_r, Tfrespective_b, Tfgeneral_g)')
...
ax2.set_title(algo+' - Left Ankle_gt')
...
ax4.set_title(algo+' - Left Angle (gt_l, Tfrespective_b, Tfgeneral_g)')
