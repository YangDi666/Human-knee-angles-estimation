# Angles++ : Walking model for correcting the bad frames - PifPaf
# Usage: python3 correcte_pp.py <nb_video> <direction(front/back)> <filter(gaussian/mean/median)> <filter kernel size> <showfigures> ex : 1 front gaussian 5 True')
...
if __name__=='__main__':
    if(len(sys.argv)!=6):
        print('usage: python3 correcte_pp.py <nb_video> <direction(front/back)> <filter(gaussian/mean/median)> <filter kernel size> <showfigures> ex : 1 front gaussian 5 True')
    ...
    for i in files:
        if (len(re.findall('PifPaf_joints_3DKinect_.'+direction[1:]+'2', i))!=0):
            filecm=i
    print(filecm)   
    ...
    correct1(k['z_lkn'], k['bad_frame'], 'kn', 1)
    print('----------')
    correct1(k['z_rkn'], k['bad_frame'], 'kn', 1)
    ...
    for i in range(len(k['frames'])):
        k['kangle_l'][i]=180-tools_pp.angle((k['x_lak'][i],k['y_lak'][i],k['z_lak'][i]),(k['x_lkn'][i],k['y_lkn'][i],k['z_lkn'][i]),(k['x_las'][i],k['y_las'][i],k['z_las'][i]), True)
        k['kangle_r'][i]=180-tools_pp.angle((k['x_rak'][i],k['y_rak'][i],k['z_rak'][i]),(k['x_rkn'][i],k['y_rkn'][i],k['z_rkn'][i]),(k['x_ras'][i],k['y_ras'][i],k['z_ras'][i]), True) 
    fig3=plt.figure()
    ...
    if direction=='back':
        k.to_csv('testVideos'+'/test'+nb_video+'/'+number+'_PifPaf_joints_3DKinect_back3.csv',encoding='gbk')
    else:
        k.to_csv('testVideos'+'/test'+nb_video+'/'+number+'_PifPaf_joints_3DKinect_front.csv',encoding='gbk')
